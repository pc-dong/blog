<template><div><h1 id="通过自定义注解实现rest风格的rsocket-crud应用" tabindex="-1"><a class="header-anchor" href="#通过自定义注解实现rest风格的rsocket-crud应用" aria-hidden="true">#</a> 通过自定义注解实现REST风格的RSocket CRUD应用</h1>
<h2 id="背景" tabindex="-1"><a class="header-anchor" href="#背景" aria-hidden="true">#</a> 背景</h2>
<blockquote>
<p><a href="https://rsocket.io/" target="_blank" rel="noopener noreferrer">RSocket<ExternalLinkIcon/></a>是一种新的应用层二进制的点对点通信协议，被认为<a href="https://www.infoq.com/articles/give-rest-a-rest-rsocket/" target="_blank" rel="noopener noreferrer">比HTTP更适合于微服务应用<ExternalLinkIcon/></a>。基于Reactive Streams规范设计，具有双向异步通信、多路复用、消息驱动、支持应用层背压流量控制、断点续传等特性。传输层协议基于抽象设计，可以在TCP、WebSocket等协议间切换，因此可以应用于Web应用、物联网、移动通信等多种场景。 由Facebook、Netifi和Pivotal等公司工程师开发，提供了Java，JavaScript，C++和Kotlin等多语言实现，其中Java版本基于Reactor框架实现，可以方便和Webflux结合使用。目前该协议第一个最终正式版本尚未发布，但已经得到了很多公司的支持和投入，Dubbo从版本3开始也针对RSocket进行了适配，因此值得学习。</p>
</blockquote>
<p>Spring也对RSocket应用开发做了很多支持，可以方便地开发出RSocket应用。但是RSocket是一种消息驱动协议，仅能通过消息路径（router）区分接口，没有类似Http中Method区分CRUD操作，需要在资源路径中添加操作前缀或后缀区分对资源的操作类型，本文给出了一种通过自定义注解的方式添加统一后缀的方法，可以比较优雅地定义REST风格的RSocket CRUD接口。</p>
<h2 id="rsocket交互模式" tabindex="-1"><a class="header-anchor" href="#rsocket交互模式" aria-hidden="true">#</a> RSocket交互模式</h2>
<p>RSocket协议定义了四种交互模式：</p>
<ul>
<li>Request/Response：单个请求接收单个响应，类似Http的请求/响应模式，但它是异步非阻塞和多路复用的。</li>
</ul>
...
<li>Fire-and-Forget：发送请求不用等待响应。适用于非关键事件日志、埋点上报等不关心响应的场景，相比于Http可以减少等待响应的网络和应用资源浪费。</li>
</ul>
...
<li>Request/Stream：单个请求可以接收多个响应。可应用于列表资源获取，服务端可在单个资源准备好后即返回，无需等所有资源都准备好，可以节约服务端缓存占用。</li>
</ul>
...
<li>Channel: 该模式下客户端可以发送多个请求数据同时接收多个响应。在此模型中，消息流在两个方向上异步流动。例如客户端将文件分片多次上传，服务端返回每次的上传结果。</li>
</ul>
...
<p>RSocket协议支持双向通信，因此建立链接后，服务端也可以主动向客户端发起请求。</p>
<p>RSocket是消息驱动的异步通信协议，徐昊老师在《如何落地业务建模》课程中讲解过，<a href="https://time.geekbang.org/column/article/397723" target="_blank" rel="noopener noreferrer">异步调用可以避免弹性耦合<ExternalLinkIcon/></a>,因此采用RSocket可以从通信协议层面实现弹性解耦。</p>
</blockquote>
<h2 id="基于spring实现rsocket-服务端api与消费" tabindex="-1"><a class="header-anchor" href="#基于spring实现rsocket-服务端api与消费" aria-hidden="true">#</a> 基于Spring实现RSocket 服务端API与消费</h2>
<h3 id="服务端api实现" tabindex="-1"><a class="header-anchor" href="#服务端api实现" aria-hidden="true">#</a> 服务端API实现</h3>
<ul>
<li>添加依赖</li>
</ul>
...
<li>添加Controller类，并添加@Controller注解</li>
</ul>
...
<p>Spring中通过@MessageMapping注解声明RSocket API，API之间通过消息路径（router）
区分。
Request/Response模式方法举例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@MessageMapping</span><span class="token punctuation">(</span><span class="token string">"toUpperCase"</span><span class="token punctuation">)</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Fire-and-Forget模式方法举例:</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@MessageMapping</span><span class="token punctuation">(</span><span class="token string">"log"</span><span class="token punctuation">)</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Request/Stream模式方法举例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@MessageMapping</span><span class="token punctuation">(</span><span class="token string">"splitString"</span><span class="token punctuation">)</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Channel模式方法举例：</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="rsocket-api消费" tabindex="-1"><a class="header-anchor" href="#rsocket-api消费" aria-hidden="true">#</a> RSocket API消费</h3>
<h4 id="rsc命令行工具" tabindex="-1"><a class="header-anchor" href="#rsc命令行工具" aria-hidden="true">#</a> RSC命令行工具</h4>
<p><a href="https://github.com/making/rsc" target="_blank" rel="noopener noreferrer">RSC<ExternalLinkIcon/></a>是类似与curl 命令的RSocket 命令客户端工具， 方便对RSocket API进行调试，具体安装使用方法可以查看<a href="https://github.com/making/rsc" target="_blank" rel="noopener noreferrer">GitHub - RSocket Client CLI (RSC)
<ExternalLinkIcon/></a>。这里分别给出四种通信模式的命令以及返回示例：</p>
<ul>
<li>Request/Response</li>
</ul>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code> $ rsc <span class="token parameter variable">--route</span><span class="token operator">=</span>toUpperCase <span class="token parameter variable">--request</span> <span class="token parameter variable">-m</span> <span class="token number">123</span> <span class="token parameter variable">--mmt</span> message/x.client.id <span class="token parameter variable">--data</span><span class="token operator">=</span>Sunday tcp://localhost:7001
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>$ rsc --route=channelToUpperCase --channel --data=- tcp://localhost:7001 

>hello
{"message":"HELLO"}
>rsocket
{"message":"RSOCKET"}
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="通过spring中提供的rsocketrequester消费rsocket-api" tabindex="-1"><a class="header-anchor" href="#通过spring中提供的rsocketrequester消费rsocket-api" aria-hidden="true">#</a> 通过Spring中提供的RSocketRequester消费RSocket API</h4>
<ul>
<li>将RSocketRequester定义为Spring Bean</li>
</ul>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>@Bean
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>// Request/Response
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="基于spring实现rest风格crud接口" tabindex="-1"><a class="header-anchor" href="#基于spring实现rest风格crud接口" aria-hidden="true">#</a> 基于Spring实现REST风格CRUD接口</h2>
<p>RSocket API 只能通过接口路径（router）区分不同接口，没有Http协议中的Method来区分CRUD操作，因此Spring中也未提供类似@GettingMapping注解。要实现REST风格的接口，需要在资源路径上添加操作前缀或后缀，为了减少重复代码和便于项目统一，我想到可以通过自定义注解的方式来实现，下面给出关键实现代码。</p>
<ul>
<li>添加自定义CRUD方法对应注解：</li>
</ul>
...
<li>定义MyRSocketMessageHandler继承RSocketMessageHandler，重写getCondition方法</li>
</ul>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>public class MyRSocketMessageHandler extends RSocketMessageHandler {
...
<li>声明RSocketMessageHandler Bean 替换默认RSocketMessageHandler实现</li>
</ul>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>@Bean
@Primary
...
<p>至此就可以通过自定义注解优雅地定义REST风格的RSocket CRUD接口了，示例代码如下：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@Controller</span>
...
<p>本文简单介绍了RSocket的一些背景知识，通过Spring怎么创建RSocket应用以及RSocket接口如何消费，并给出了一种在Spring中基于自定义注解方式优雅定义REST风格的RSocket接口的方法，相关代码见<a href="https://github.com/pc-dong/rsocket-demos" target="_blank" rel="noopener noreferrer">GitHub - pc-dong/rsocket-demos<ExternalLinkIcon/></a>。</p>
</div></template>
